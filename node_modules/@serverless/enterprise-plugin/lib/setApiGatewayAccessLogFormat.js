...
const _require = require('./utils.js'),
      API_GATEWAY_LOG_FORMAT = _require.API_GATEWAY_LOG_FORMAT,
      API_GATEWAY_V2_LOG_FORMAT = _require.API_GATEWAY_V2_LOG_FORMAT;
...
const setupHttpLogs = serviceConfig => {
  ...
  if (!_.values(serviceConfig.functions).some(functionData => functionData.events.some(event => event.http))) {
...
const setupHttpApiLogs = serviceConfig => {
  ...
  if (!_.values(serviceConfig.functions).some(functionData => functionData.events.some(event => event.httpApi))) {
...
module.exports = ctx => {
  ...
  if (serviceConfig.custom && serviceConfig.custom.enterprise && serviceConfig.custom.enterprise.collectApiGatewayLogs === false) {
    return;
  }
...
//# sourceMappingURL=setApiGatewayAccessLogFormat.js.map